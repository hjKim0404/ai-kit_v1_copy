...
from utils.helper_imaug import random_patch, random_imaug
...
class TightFaceProvider(Sequence):
    ...
    """
    def __init__(self, fg_folder, bg_folder, batch_size, dark_weight=None, light_weight=None):
        """
        ...
        # 사이즈별로 나온 월리 얼굴들에 밝기가 기본, 어두운 것, 밝은 것으로 이미지 생성
        self.fg_imgs = random_imaug(self.fg_imgs, dark_weight, light_weight)
    ...
    def __getitem__(self, index):
        ...
        indices = np.random.choice(np.arange(len(self.fg_imgs)), size=fg_batch, replace=False)
