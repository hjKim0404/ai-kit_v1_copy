...
def zero_padding(image, max_h, max_w):
    ...
    :param max_h: padding 후 이미지 height 의 크기
    :param max_w: padding 후 이미지 width 의 크기
...
def cropper(img, stride_h=35, stride_w=35, filter_h=35, filter_w=35):
    ...
    # coordinates : 해당 Crop된 이미지에 mapping 되는 좌표(x,y)를 저장하니다.
    coordinates = []
    ...
    for y in h_indices:
        for x in w_indices:
            ...
            # Window가 이미지 마지막 부분에 가면 지정한 크기보다 작아질수 있어서 그럼 해당 부분은 저장하지 않는다.
            if (crop_img.shape[:2]) == (filter_h, filter_w):
                crop_imgs.append(img[y: y + filter_h, x: x + filter_w])

                # 정상적인 이미지의 좌표값을 저장합니다, (x1, y1, x2, y2)
...
def images_cropper(images, stride_h=35, stride_w=35, filter_h=35, filter_w=35):
    ...
    bucket_coords = []
    for image in images:
        cropped_images, cropped_coords = \
            cropper(image, stride_h=stride_h, stride_w=stride_w, filter_h=filter_h, filter_w=filter_w)
        bucket_images.append(cropped_images)
        bucket_coords.extend(cropped_coords)
