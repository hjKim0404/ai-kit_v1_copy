...
class TightFaceProvider(Sequence):
    ...
    def __getitem__(self, index):
        ...
        # 여자친구 이미지 1개 무작위 선택
        girl_indices = np.random.choice(np.arange(len(self.girl_imgs)), 1)
        girl_img = [self.girl_imgs[ind] for ind in girl_indices][0]
        
        # background 이미지 중 1개에 여자친구 이미지 random patch 적용
        patch_girl_ind = np.random.choice(np.arange(len(bg_imgs)), 1)
        bg = bg_imgs[patch_girl_ind][0].copy()
        bg_imgs[patch_girl_ind] = random_patch(bg, girl_img)
        ...
        for ind, img in enumerate(self.bg_imgs[start: end].copy()):
