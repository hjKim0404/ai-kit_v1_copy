...
def random_imaug(fg_img):
    ...
    """
    sometimes = lambda aug: iaa.Sometimes(0.1, aug) # 리스트의 이미지에 imgaug가 적용될 확률 10%
    images = fg_img # 리스트형태의 이미지를 images 변수에 저장
    
    # imgaug 적용 함수
    imgaug = iaa.Sequential([
        sometimes(iaa.SomeOf((1, 2),[ # 1개에서 2개의 imgaug를 적용
            iaa.GaussianBlur(sigma=(0, 0.5)),  # 가우시안 흐림 효과 적용
            iaa.Multiply((0.9, 1.2)), # 밝기 효과
            iaa.LinearContrast((1, 2)), # 대비 효과
        ]))
        
    ], random_order=True)  # 무작위 순서로 적용 
    
    # 리스트로 들어온 원소에 imgaug 적용
    imgaug = imgaug(images=images)

    return imgaug


def back_gaussian_blur(bg_img):
    """
    Describes:
        포그라운드 이미지들에 여러가지 효과들을 적용시킨 후 저장하는 함수입니다.
        참고: https://github.com/aleju/imgaug (패키지 깃헙)
