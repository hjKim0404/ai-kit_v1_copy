...
def random_imaug(fg_img, dark_weight, light_weight):
    """
    Describes:
    ...
    dark = iaa.Sequential([
        ...
        iaa.Multiply((dark_weight, 0.9))
        
    ], random_order=True)  # apply not augmenters in random order
    # 기본 이미지에 밝기 밝게 1.1~2.5배 밝게 만듬
    light = iaa.Sequential([
        #   iaa.GaussianBlur(sigma=(0, 2)),
        iaa.Multiply((1.1, light_weight))
        
    ], random_order=True)  # apply not augmenters in random order
    
    # 기본 이미지, 밝은 이미지, 어두운 이미지 리스트로 묶음
    light_images_aug = light(images=images)
    dark_images_aug = dark(images=images)    
    images += light_images_aug
    images += dark_images_aug
    return images


def read_canny(paths):
    bucket_imgs = []
    for path in paths:
        src = cv2.imread(path)
#        blur = cv2.GaussianBlur(src, (5,5), 1)
#        gray = cv2.cvtColor(src, cv2.COLOR_RGB2GRAY)
        sobel = cv2.Canny(src, 50, 100)
        bucket_imgs.append(sobel)
    return bucket_imgs