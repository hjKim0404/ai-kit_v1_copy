...
from utils.helper_girls_imgaug import random_imaug
...
class TightFaceProvider(Sequence):
    ...
    def __init__(self, wally_folder, girl_folder, bg_folder, batch_size):
        ...
        # self.aug_wally_imgs = random_imaug(self.wally_imgs.copy()) 
    ...
    def __getitem__(self, index):
        ...
        girl_indices = np.random.choice(np.arange(len(self.girl_imgs)), girl_size)
        ...
        indices = np.random.choice(np.arange(len(self.wally_imgs)), size=fg_batch)
        ...
        # 랜덤하게 월리 얼굴을 가져오기 위한 fg_batch 개수 만큼의 인덱스를 무작위로 가져온다.
        # 위에서 지정한 랜덤 인덱스에 맞는 월리 얼굴을 가져온다.
        aug_fg_imgs = random_imaug([self.wally_imgs[ind] for ind in indices].copy())
        # 길이에 맞게 라벨을 붙인다.
        # foreground label: 1
        ...
        batch_xs = np.concatenate([aug_fg_imgs, girl_bg_imgs, bg_imgs], axis=0)
        # batch_ys에는 background, foreground의 라벨들이 들어간다.
        batch_ys = np.concatenate([aug_fg_labs, girl_bg_labs, bg_labs], axis=0)
        return batch_xs, batch_ys

    
class TightFaceProvider2(Sequence):
    """
    해당 클래스는 월리 얼굴을 Tight 하게 잘라 만든 데이터를 활용하기 위해 만들어진 DataGenerator 입니다.
    # Foreground 생성
        1. tight 이미지를 이미지내 모든 h, w에 대해 resize 합니다.
    ...
    def __getitem__(self, index):
        ...
        # batch_xs에는 background, foreground의 이미지들이 들어간다.
        batch_xs = np.concatenate([aug_fg_imgs, bg_imgs], axis=0)
        # batch_ys에는 background, foreground의 라벨들이 들어간다.
        batch_ys = np.concatenate([aug_fg_labs, bg_labs], axis=0)
        return batch_xs, batch_ys

class TightFaceProvider3(Sequence):
    """
    해당 클래스는 월리 얼굴을 Tight 하게 잘라 만든 데이터를 활용하기 위해 만들어진 DataGenerator 입니다.
    # Foreground 생성
        1. tight 이미지를 이미지내 모든 h, w에 대해 resize 합니다.
